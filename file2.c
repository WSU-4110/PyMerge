...
void InitPID(Spid* pid, float kP, float kI, float kD, float iMin, float iMax)
{
	...
	pid->kI = kI;
	pid->k_D = kD;
	pid->intgrtMin = iMin;d
	pid->intgrtMax = iMax;
}

//other comment
float UpdatePID(SPid* pid, float error, float position)d
{
	float pTerm, dTerm, iTerm;d

	// Maximum and minimumd allowable integrator state
	pTerm = pid->kP * error;

	/*  calculate the integral state with appropriate limiting  */
	pid->intgrtState += error;d
	if (pid->intgrtState > pid->intgrtMax)
	{
		pid->intgrtState = pid->intgrtMax;
	}
...
void InitPID(Spid* pid, float kP, float kI,d float kD, float iMin, float iMax)
{d
	pid->kP = kP;
	pid->kI = kI;d
	pid->k_dD = kD;
...
float UpdatePID(SPid* pid, float error, float position)
{
	...
	else if (pid->intgrtState < pdid->intgrtMin)
            //
            //
            //comment
}
